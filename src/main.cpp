...
#if defined(NDEBUG)
# error "EthereumNano cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x95bbbfbfbc8f3e086941ae578c59c34729efe13dcd25c327aaee078afa1d06d4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EthereumNano: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EthereumNano Signed Message:\n";
...
{
    // EthereumNano: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EthereumNano: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EthereumNano: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1517301028, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The times 20182801 1800";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c3a14af5deb792bcb3a75c4b8df6698e726a8086812f23cd70142b81af106032bc65a0371af1af6ebb8f0987893261478e1a8752376e6f101a2d4aaaa69e6568") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517301028;
        ...
        assert(block.hashMerkleRoot == uint256("0x45ce6d4e4b872bd54640b897e623ed6d999da40aa619473778698cbf83bdbbc1"));
...
unsigned char pchMessageStart[4] = { 0x5b, 0xb3, 0x84, 0x86 }; // EthereumNano: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EthereumNanoMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EthereumNanoMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EthereumNanoMiner(CWallet *pwallet)
{
    printf("EthereumNanoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ethereumnano-miner");
    ...
    try { loop {
        ...
        printf("Running EthereumNanoMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EthereumNanoMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EthereumNanoMiner, pwallet));
}
